...
export async function registerDispatcherShiftRoutes(app: FastifyInstance) {
  app.get("/api/v1/public/ordering-status", async (_request, reply) => {
    reply.header("Cache-Control", NO_STORE_HEADER);
  ...
  app.get("/api/v1/dispatcher/workspace", { preHandler: requirePermission("manage_dispatcher_shift") }, async (request, reply) => {
  ...
  app.get("/api/v1/dispatcher-shifts", { preHandler: requirePermission("view_dispatcher_shifts") }, async (request, reply) => {
    const user = request.authUser;
  ...
  app.get("/api/v1/dispatcher-shifts/:shiftId", { preHandler: requirePermission("view_dispatcher_shifts") }, async (request, reply) => {
    const user = request.authUser;
    ...
    return {
      data: await getDispatcherShiftDetail(user, getNumericParam(request, "shiftId")),
    };
  });
}
